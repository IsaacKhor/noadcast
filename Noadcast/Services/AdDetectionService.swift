...
/// request to Gemini, the response is parsed against
...
actor AdDetectionService {
    ...
    You are analyzing a podcast transcript and identifying segments the listener
    probably wants to skip. Each segment you return has a `kind`:

    1. "intro": a single contiguous segment at or near the very BEGINNING of the
    episode covering theme music, branding, preroll ads, and any host
    introductions and show introductions that are likely the same between
    episodes. The intro is played before the main content starts, and there is
    at most one intro per episode, and it spans from the start of the episode
    through to where the substantive content begins. Do NOT include host,
    interviewee, guest, or episode introductions that are specific to this
    episode in the intro.

    2. "ad": a mid-episode advertisement, sponsored message, host-read ad, promo
    code, paid endorsement, or cross-promotion of another podcast that appears
    BETWEEN the intro and outro (i.e. inside the main content). Editorial
    mentions, listener mail, the host's own products discussed editorially, and
    interview segments are NOT ads. Mulitple ad segments adjacent to each other
    should be merged into a single segment covering the entire stretch of ads,
    but only if they are not separated by substantive content.

    3. "outro": a single contiguous segment at the very END of the episode
    covering closing music, credits, next-episode teasers, postroll ads, and
    farewells. There is at most one outro per episode, and it spans from where
    the substantive content finishes through to the end of the episode. Intros
    and outros may include ads — they're still a single intro/outro segment, not
    separate entries.

    Given the timestamped transcript lines, return the start and end seconds of
    each segment using only timestamps that appear in the transcript. Be
    conservative — flag ads only when you're confident. Return an empty list if
    ...
    func detectAds(
        ...
        let usage: TokenUsage?
        switch provider {
        case .gemini3Flash, .gemini35Flash, .gemini31FlashLite, .gemini25Flash, .gemini25FlashLite:
